...
import { DeliverySheetService } from "@/services/DeliverySheetService";
...
const OrderEntry = () => {
  ...
  const [autoGenerateDeliverySheet, setAutoGenerateDeliverySheet] = useState(true);
  ...
  // Auto-assign product codes on component mount
  useEffect(() => {
    assignMissingProductCodes();
  }, []);
  ...
  const assignMissingProductCodes = () => {
    const productsWithoutCodes = products.filter(p => !p.code || p.code.trim() === '');
    
    if (productsWithoutCodes.length > 0) {
      productsWithoutCodes.forEach(product => {
        const newCode = ProductCodeService.generateCode(product.name, products);
        updateProduct(product.id, { code: newCode });
      });
      
      toast.success(`Assigned codes to ${productsWithoutCodes.length} products`);
    }
  };
  ...
  const saveOrder = () => {
    ...
    // Auto-generate delivery sheet if enabled
    if (autoGenerateDeliverySheet && savedOrder) {
      try {
        const deliverySheet = DeliverySheetService.generateDeliverySheet(
          [savedOrder],
          customers,
          products,
          orderDate,
        ...
        if (deliverySheet.rows.length > 0) {
          addTrackSheet(deliverySheet);
          toast.success("Delivery sheet auto-generated");
        }
      } catch (error) {
        console.error("Error generating delivery sheet:", error);
        toast.error("Failed to auto-generate delivery sheet");
      }
    }
  ...
  const handleAddProduct = () => {
    ...
    const productCode = ProductCodeService.generateCode(productName, products);
    ...
    const newProduct = {
      ...
      isActive: true,
      code: productCode // Add auto-generated code
    };
    ...
    if (editingProduct) {
      ...
      toast.success(`Product added with code: ${productCode}`);
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Create and manage daily milk orders with automatic delivery sheet generation
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={assignMissingProductCodes}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  id="autoDeliverySheet"
                  checked={autoGenerateDeliverySheet}
                  onChange={(e) => setAutoGenerateDeliverySheet(e.target.checked)}
                  className="rounded"
                />
                <label htmlFor="autoDeliverySheet" className="text-sm">
                  Auto-generate delivery sheet
                </label>
              </div>
                  ...
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      {editingProduct 
                        ...
                        : "Add product details to create a new product. Code will be auto-generated."
                      }
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                      ...
                      <div key={product.id} className="flex items-center rounded-md bg-gray-100 px-3 py-2">
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            ...
                            {product.code && <span className="ml-2 text-blue-600">({product.code})</span>}
                          </div>
                        </div>
                        <Button
                          variant="ghost"
                    ...
                    {products.map(product => (
                      <TableRow key={product.id}>
                        <TableCell className="font-medium">
                          ...
                          {product.code && <div className="text-xs text-blue-600">Code: {product.code}</div>}
                        </TableCell>
                        {customers.map(customer => {
